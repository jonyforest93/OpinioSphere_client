...
const Post = (props) => {
    ...
    return (
        <div
            className="flex flex-col items-start p-8 rounded-xl hover:shadow-xl shadow-gray-900"
            onClick={() => navigate(`/review/${props._id}/details`)}
        >
            ...
            <div className="mt-10 flex justify-start">
                <span
                    className={`${isDarkTheme ? 'text-textDarkSec bg-textDarkSec bg-opacity-10' : 'text-badge bg-purple-100'} text-base  font-inter px-4 py-2 rounded-xl mr-3`}
                >{props.category}
                </span>
                {
                    !!props.rateCount &&
                    <span
                        className={`${isDarkTheme ? 'text-textDarkSec bg-textDarkSec bg-opacity-10' : 'text-badge bg-purple-100'} text-base  font-inter px-4 py-2 rounded-xl mr-3 flex justify-start items-center`}
                    >
                        {(props.rateSum / props.rateCount).toFixed(2)}
                        <span className="ml-1">
            ...
            <div className={`${isDarkTheme ? 'text-textDark' : 'text-gray-900'} mt-4`}>
                <h3
